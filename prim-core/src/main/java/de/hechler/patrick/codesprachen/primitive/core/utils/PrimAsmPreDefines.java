...
public class PrimAsmPreDefines {
	 ...
	 * <code>X01</code> will be <code>-1</code> if the memory-block could not be reallocated, the old memory-block will remain valid and should be freed if it is not
	 * longer needed<br>
	 * <code>X01</code> will point to the new memory block, the old memory-block was automatically freed, so it should not be used, the new block should be freed if
	 * it is not longer needed<br>
	 * the value can be used by the <code>INT</code> command to indicate that this interrupt should be called
	 */
	 ...
	 * <li>note that when <code>X01</code> does not point to the start of the memory block the start of the memory block can still be moved during the
	 * reallocation</li>
	 * </ul>
	 ...
	 * <ul>
	 * <li>the new length will be the old length or if the old length is smaller than the size of the STRING (with <code>\0</code>) than the size of the STRING (with
	 * <code>\0</code>)</li>
	 * </ul>
	 * on error <code>X01</code> will be set to <code>-1</code><br>
	 * the value can be used by the <code>INT</code> command to indicate that this interrupt should be called
	 ...
	 * <li>if <code>ERRNO</code> is set to out of range, the string value displayed a value outside of the 64-bit number range and <code>X00</code> will either be
	 * min or max value</li>
	 * </ul>
	 * the value can be used by the <code>INT</code> command to indicate that this interrupt should be called
	 */
	 ...
	 * <code>X02</code> is set to the length of the buffer<br>
	 * <code>X00</code> points to the start of the unconverted sequenze (or behind the <code>\0</code> terminator (note that the <code>\0</code> char needs two
	 * bytes))<br>
	 * <code>X01</code> points to the start of the unmodified space of the target buffer<br>
	 * <code>X02</code> will be set to unmodified space at the end of the buffer<br>
	 * <code>X03</code> will be set to the number of converted characters or <code>-1</code> on error<br>
	 ...
	 * <code>X01</code> points to the buffer to be filled with the converted STRING (<code>UTF-8</code>)<br>
	 * <code>X02</code> is set to the length of the buffer<br>
	 * <code>X00</code> points to the start of the unconverted sequenze (or behind the <code>\0</code> terminator (note that the <code>\0</code> char needs four
	 * bytes))<br>
	 * <code>X01</code> points to the start of the unmodified space of the target buffer<br>
	 * <code>X02</code> will be set to unmodified space at the end of the buffer<br>
	 * <code>X03</code> will be set to the number of converted characters or <code>-1</code> on error<br>
	 ...
	 * <ul>
	 * <li>if the pointer is not <code>-1</code> the pointer will be converted by placing a <code>"p-"</code> and then the unsigned pointer-number converted to a
	 * STRING using the hexadecimal number system</li>
	 * <li>if the pointer is <code>-1</code> it will be converted to the STRING <code>"p-inval"</code></li>
	 * </ul>
	 * </li>
	 * <li><code>%h</code>: the next argument contains a number, which should be converted to a STRING using the hexadecimal number system and than be inserted
	 * here</li>
	 ...
	 * <code>X01</code> will be set to the length of the file (and the memory block)<br>
	 * <code>X02</code> will be set to <code>1</code> if the file has been loaded as result of this interrupt and <code>0</code> if the file was previously
	 * loaded<br>
	 * when an error occurred <code>X01</code> will be set to <code>-1</code>, <code>ERRNO</code> will be set to a non-zero value and X00 will be unmodified<br>
	 * the value can be used by the <code>INT</code> command to indicate that this interrupt should be called
	 */
