...
public class PrimAsmPreDefines {
	 ...
	 * <li>exits with <code>(128 + illegal_interrup_number)</code> (without calling
	 * the exit interrupt)</li>
	 * <li>if this interrupt is tried to bee called, but it is forbidden to call
	 * this interrupt, the program exits with
	 * <code>128</code></li>
	 * </ul>
	 */
	 ...
	 * <li>if the value of <code>X00` is `-1</code> after the call the memory-block
	 * could not be allocated</li>
	 * <li>if the value of <code>X00` is not `-1`, `X00</code> points to the first
	 * element of the allocated
	 * memory-block</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01` will be `-1</code> if the memory-block could not be
	 * reallocated, the old memory-block will remain
	 * valid and should be freed if it is not longer needed</li>
	 * <li><code>X01</code> will point to the new memory block, the old memory-block
	 * was automatically freed, so it
	 * should not be used, the new block should be freed if it is not longer
	 * needed</li>
	 * </ul>
	 */
	 ...
	 * open new stream
	 * <ul>
	 * <li><code>X00</code> contains a pointer to the STRING, which refers to the
	 * file which should be read</li>
	 ...
	 * <li>when this flags is set either <code>OPEN_FILE` or `OPEN_PIPE</code> has
	 * to be set</li>
	 * <li><code>OPEN_ALSO_CREATE</code></li>
	 * <li>create the file/pipe if it does not exist, but do not fail if the
	 * file/pipe exist already (overwritten by
	 * PFS_SO_ONLY_CREATE)</li>
	 * <li><code>OPEN_FILE</code></li>
	 * <li>fail if the element is a pipe and if a create flag is set create a file
	 * if the element does not exist
	 * already</li>
	 * <li>this flag is not compatible with <code>OPEN_PIPE</code></li>
	 * <li><code>OPEN_PIPE</code></li>
	 * <li>fail if the element is a file and if a create flag is set create a
	 * pipe</li>
	 ...
	 * <li>open the stream for append access (before every write operation the
	 * position is set to the end of the
	 * file)</li>
	 ...
	 * <li>the operation will fail if it is not spezified if the file should be
	 * opened for read, write and/or
	 * append</li>
	 * <li>opens a new stream to the specified file</li>
	 * <li>if successfully the STREAM-ID will be saved in the <code>X00</code>
	 * register</li>
	 * <li>if failed <code>X00` will contain `-1</code></li>
	 * <li>to close the stream use the stream close interrupt
	 * (<code>INT_STREAM_CLOSE</code>)</li>
	 * </ul>
	 */
	 ...
	 * <li>after execution <code>X01</code> will contain the number of elements,
	 * which has been read.</li>
	 * <li>when the value is less than len either an error occured or end of
	 * file/pipe has reached (which is not
	 ...
	 * <li>note that it is possible to set the stream position behind the end of the
	 * file.</li>
	 * <li>when this is done, the next write (not append) operation will fill the
	 * hole with zeros</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00</code> contains the STREAM/FILE_STREAM-ID</li>
	 * <li><code>X01</code> contains the value, which should be added to the
	 * position of the stream</li>
	 * <li><code>X01` is allowed to be negative, but the sum of the old position and `X01</code>
	 ...
	 * <ul>
	 * <li><code>X00</code> contains the STREAM-ID</li>
	 * <li><code>X01</code> will be set to the new position of the stream or -1 on
	 * error</li>
	 * <li>sets the position of the stream to the end of the file (the file
	 * length)</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00` points to the `STRING</code> which contains the path of the
	 * file to be opened</li>
	 * <li><code>X00</code> will be set to the newly opened STREAM/FILE-ID or -1 on
	 * error</li>
	 ...
	 * <ul>
	 * <li><code>X00` points to the `STRING</code> which contains the path of the
	 * folder to be opened</li>
	 * <li><code>X00</code> will be set to the newly opened STREAM/FOLDER-ID or -1
	 * on error</li>
	 ...
	 * <ul>
	 * <li><code>X00` points to the `STRING</code> which contains the path of the
	 * pipe to be opened</li>
	 * <li><code>X00</code> will be set to the newly opened STREAM/PIPE-ID or -1 on
	 * error</li>
	 ...
	 * <ul>
	 * <li><code>X00` points to the `STRING</code> which contains the path of the
	 * element to be opened</li>
	 * <li><code>X00</code> will be set to the newly opened STREAM-ID or -1 on
	 * error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X00</code> will be set to the newly opened ELEMENT/FOLDER-ID or -1
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li>note that
	 * <code>-1` may be the create date of the element, so check `ERRNO</code>
	 * instead</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00</code> contains the ELEMENT-ID</li>
	 * <li><code>X01` will be set to the last modified date or `-1</code> on
	 * error</li>
	 * <li>note that
	 * <code>-1` may be the last modified date of the element, so check `ERRNO</code>
	 * instead</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00</code> contains the ELEMENT-ID</li>
	 * <li>note that this operation automatically closes the given ELEMENT-ID, the
	 * close interrupt should not be invoked
	 ...
	 * <ul>
	 * <li><code>X00</code> contains the ELEMENT-ID</li>
	 * <li><code>X01` points to a STRING which will be the new name or it is set to `-1</code>
	 * if the name should not be
	 * changed</li>
	 * <li><code>X02` contains the ELEMENT-ID of the new parent of `-1</code> if the
	 * new parent should not be
	 * changed</li>
	 * <li>when both <code>X01` and `X02` are set to `-1</code> this operation will
	 * do nothing</li>
	 * <li><code>X01` will be set to `1` or `0</code> on error</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00</code> contains the ELEMENT-ID</li>
	 * <li><code>X01</code> points the the a memory block, which should be used to
	 * store the name as a STRING</li>
	 * <li>when <code>X01` is set to `-1</code> a new memory block will be
	 * allocated</li>
	 * <li>on success <code>X01</code> will point to the name as STRING
	 * representation</li>
	 * <li>when the memory block is not large enugh, it will be resized</li>
	 * <li>note that when <code>X01</code> does not point to the start of the memory
	 * block the start of the memory block
	 ...
	 * <li>note that only the low 32 bit will be used and the high 32 bit will be
	 * ignored</li>
	 * <li><code>X01` will be set to `1` or `0</code> on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01` will be set to the number of child elements the folder has or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01` will be set to a newly opened ELEMENT-ID for the child or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01` will be set to a newly opened ELEMENT/FOLDER-ID for the child or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01` will be set to a newly opened ELEMENT/FILE-ID for the child or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01` will be set to a newly opened ELEMENT/PIPE-ID for the child or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X00</code> contains the ELEMENT/FOLDER-ID</li>
	 * <li><code>X00</code> points to a STRING with the name of the child</li>
	 * <li><code>X01` will be set to a newly opened/created ELEMENT/FOLDER-ID for the child or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01` will be set to a newly opened/created ELEMENT/FILE-ID for the child or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X00</code> contains the ELEMENT/FOLDER-ID</li>
	 * <li><code>X01</code> points to the STRING name of the new child element</li>
	 * <li><code>X01` will be set to a newly opened/created ELEMENT/PIPE-ID for the child or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00</code> contains the ELEMENT/FOLDER-ID</li>
	 * <li><code>X01` is set to `0</code> if hidden files should be skipped and any
	 * other value if not</li>
	 ...
	 * <li><code>X01` will be set to the file length in bytes or `-1</code> on
	 * error</li>
	 * </ul>
	 */
	 ...
	 * <li>this interrupt will append zeros to the file when the new length is
	 * larger than the old length or remove all
	 ...
	 * <li>note that this interrupt works for both files and pipes, but will fail
	 * for folders</li>
	 ...
	 * <li><code>X01` will be set to the pipe length in bytes or `-1</code> on
	 * error</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01</code> will be set to the curent system time in seconds since
	 * the epoch</li>
	 * <li><code>X02</code> will be set to the additional curent system time in
	 * nanoseconds</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X02</code> will be set to the additional resolution in
	 * nanoseconds</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00` contain the number of nanoseconds to wait (only values from `0` to `999999999</code>
	 * are
	 * allowed)</li>
	 * <li><code>X01` contain the number of seconds to wait (only values greather or equal to `0</code>
	 * are
	 * allowed)</li>
	 * <li><code>X00` and `X01` will contain the remaining time (both `0</code> if
	 * it finished waiting)</li>
	 * <li><code>X02` will be `1` if the call was successfully and `0</code> if
	 * something went wrong</li>
	 * <li><code>X00</code> will not be negative if the progress waited too
	 * long</li>
	 * </ul>
	 */
	 ...
	 * <li>this interrupt will wait until the current system time is equal or after
	 * the given absolute time.</li>
	 * <li><code>X00` and `X01` will contain the remaining time (both `0</code> if
	 * it finished waiting)</li>
	 * <li><code>X02` will be `1` if the call was successfully and `0</code> if
	 * something went wrong</li>
	 ...
	 * <ul>
	 * <li><code>X00` will be set to a random non negative number or `-1</code> on
	 * error</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li>copies a block of memory</li>
	 * <li>this function makes sure, that the original values of the source block
	 * are copied to the target block (even
	 ...
	 * <li><code>X01</code> the first byte contains the value to be written to each
	 * byte</li>
	 ...
	 * <li><code>X00</code> will be set to zero if both are equal STRINGs, a value
	 * greather zero if the first is
	 * greather and below zero if the second is greather</li>
	 * <li>a STRING is greather if the first missmatching char has numeric greather
	 * value</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X01</code> is points to the buffer to be filled with the number in
	 * a STRING format</li>
	 ...
	 * <li><code>X00` will be set to the size of the STRING (without the `\0</code>
	 * terminating character)</li>
	 * <li><code>X01</code> will be set to the new buffer</li>
	 * <li><code>X03</code> will be set to the new size of the buffer</li>
	 * <li>the new length will be the old length or if the old length is smaller
	 * than the size of the STRING (with
	 ...
	 * <li><code>X01</code> points to the buffer to be filled with the number in a
	 * STRING format</li>
	 ...
	 * <li>(for example <code>10` for the decimal system or `2</code> for the binary
	 * system)</li>
	 * <li>the minimum base is <code>2</code></li>
	 * <li>the maximum base is <code>36</code></li>
	 ...
	 * <li><code>X01` points to the buffer to be filled with the to `UTF-16</code>
	 * converted string</li>
	 * <li><code>X02</code> is set to the length of the buffer</li>
	 * <li><code>X00` points to the start of the unconverted sequenze (or behind the `\0</code>
	 * terminator)</li>
	 * <li><code>X01</code> points to the start of the unmodified space of the
	 * target buffer</li>
	 * <li><code>X02</code> will be set to unmodified space at the end of the
	 * buffer</li>
	 * <li><code>X03` will be set to the number of converted characters or `-1</code>
	 * on error</li>
	 * </ul>
	 */
	 ...
	 * <ul>
	 * <li><code>X00` points to the STRING (`UTF-8</code>)</li>
	 * <li><code>X01` points to the buffer to be filled with the to `UTF-32</code>
	 * converted string</li>
	 * <li><code>X02</code> is set to the length of the buffer</li>
	 * <li><code>X00` points to the start of the unconverted sequenze (or behind the `\0</code>
	 * terminator)</li>
	 ...
	 * <li><code>X00` will be set to the length of the output string (the offset of the `\0` character) or `-1</code>
	 * on
	 * error</li>
	 * <li>if <code>X00` is larger or equal to `X02`, only the first `X02</code>
	 * bytes will be written to the
	 * buffer</li>
	 * <li>formatting:</li>
	 * <li><code>%%`: to escape an `%` character (only one `%</code> will be in the
	 * formatted STRING)</li>
	 * <li><code>%s</code>: the next argument points to a STRING, which should be
	 * inserted here</li>
	 * <li><code>%c</code>: the next argument starts with a byte, which should be
	 * inserted here</li>
	 * <li><code>%n</code>: consumes two arguments: the first is the base
	 * (<code>2..36</code>) and the second is the
	 * number to convert with the given base</li>
	 * <li><code>%d</code>: the next argument contains a number, which should be
	 * converted to a STRING using the decimal
	 * number system and than be inserted here</li>
	 * <li><code>%f</code>: the next argument contains a floating point number,
	 * which should be converted to a STRING
	 * and than be inserted here</li>
	 * <li><code>%p</code>: the next argument contains a pointer, which should be
	 * converted to a STRING. if the pointer
	 * is <code>-1</code> the value will be "p-inval", otherwise "p-" together with
	 * the hex representation of the
	 * pointer</li>
	 * <li><code>%h</code>: the next argument contains a number, which should be
	 * converted to a STRING using the
	 * hexadecimal number system and than be inserted here</li>
	 * <li><code>%b</code>: the next argument contains a number, which should be
	 * converted to a STRING using the binary
	 * number system and than be inserted here</li>
	 * <li><code>%o</code>: the next argument contains a number, which should be
	 * converted to a STRING using the octal
	 * number system and than be inserted here</li>
	 * </ul>
	 */
	 ...
	 * <li><code>X00` will point to the memory block, in which the file has been loaded or `-1</code>
	 * on error</li>
	 * <li><code>X01</code> will be set to the length of the file (and the memory
	 * block)</li>
	 * </ul>
	 */
	 ...
	 * <li>the difference is that this interrupt may remember which files has been
	 * loaded</li>
	 * <li>there are no guarantees, when the same memory block is reused and when a
	 * new memory block is created</li>
	 * <li>the other difference is that the file may only be unloaded with the
	 * unload lib interrupt (not with the free
	 * interrupt)</li>
	 * <li>the returned memory block also can not be resized</li>
	 * <li>if the interrupt is executed multiple times with the same file, it will
	 * return every time the same memory
	 * block.</li>
	 * <li>this interrupt does not recognize files loaded with the
	 * <code>64` (`INT_LOAD_FILE</code>) interrupt.</li>
	 * <li><code>X00</code> is set to the path (inclusive name) of the file</li>
	 * <li><code>X00</code> will point to the memory block, in which the file has
	 * been loaded</li>
	 * <li><code>X01</code> will be set to the length of the file (and the memory
	 * block)</li>
	 * <li><code>X02` will be set to `1` if the file has been loaded as result of this interrupt and `0</code>
	 * if the
	 ...
	 * <li>this interrupt will ensure that the given memory block will be freed and
	 * never again be returned from the
	...
	/**
	 * the initial value of the <code>INTCNT</code> register this constant holds the
	 * number of default interrupts
	 */
	...
	/**
	 * the first memory address of the register memory block note that X00 is not
	 * the first register, but IP
	 */
	...
	/**
	 * the STREAM-ID of the stdin pipe stream this stream is initially open for
	 * reading
	 */
	public static final long STD_IN                               = 0L;
	/**
	 * the STREAM-ID of the stdout pipe stream this stream is initially open for
	 * writing
	 */
	public static final long STD_OUT                              = 1L;
	/**
	 * the STREAM-ID of the stdlog pipe stream this stream is initially open for
	 * writing
	 */
	...
	/**
	 * create the file/pipe if it does not exist do not fail if the file/pipe exist
	 * already (overwritten by
	 * PFS_SO_ONLY_CREATE)
	 */
	public static final long STREAM_ALSO_CREATE                   = 0x00000002L;
	/**
	 * a file stream fail if the element is a pipe and if CREATE is set create a
	 * file
	 */
	public static final long STREAM_FILE                          = 0x00000004L;
	/**
	 * a pipe stream fail if the element is a file and if CREATE is set create a
	 * pipe
	 */
	...
	/**
	 * open the stream for append access (before every write operation the position
