...
returns [List<Command> commands, Map<String,Long> labels, long pos, AssembleRuntimeException are] @init {
 	$pos = startpos;
 	$labels = new HashMap<>();
				...
				)
				{
					...
					if ($enabled) {
						if (be) {
							...
							$are.addSuppressed(new AssembleRuntimeException($ERROR.getLine(), $ERROR.getCharPositionInLine(), (String) $zusatz));
						} else {
							$are = new AssembleRuntimeException($ERROR.getLine(), $ERROR.getCharPositionInLine(), (String) $zusatz);
						}
					}
				}
			...
			ANY
			{
				if ($enabled) {
					if (be) {
						...
						$zusatz = new AssembleRuntimeException($ANY.getLine(), $ANY.getCharPositionInLine(),"illegal character at line: " + $ANY.getLine() + ", pos-in-line: "+$ANY.getCharPositionInLine()+" char='" + $ANY.getText() + "'");
					}
				}
			}
...
[long pos, Map<String, Long> constants, boolean be]
returns [long num, AssembleRuntimeException are]
:
	(
		nummer [pos, constants, be]
		...
		NAME
		{
 			...
 			if (zw == null) {
 				if (be) {
	 				...
	 				zw = 0L;
	 				$are = new AssembleRuntimeException($NAME.getLine(), $NAME.getCharPositionInLine(), "unknown constant: '" + $NAME.getText() + "', known constants: '" + constants + "'");
 				}
 			}
 			$num = (long) zw;
 		}
