...
void* hashset_get(const struct hashset *set, unsigned int hash, const void *equalto) {
	if (!set->setsize) {
		return NULL;
	}
...
void* hashset_put(struct hashset *set, unsigned int hash, void *newvalue) {
	...
	if (set->setsize <= set->entrycount * 1.5) {
		...
		for (int i = 0; i < newsize; i++) {
			newentries[i] = NULL; //NULL entries are not permittet
		}
	...
	return put0(set, hash, newvalue);
}

void* hashset_remove(struct hashset *set, unsigned int hash, void *newvalue) {
